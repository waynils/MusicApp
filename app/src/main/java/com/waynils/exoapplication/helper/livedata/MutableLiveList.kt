package com.waynils.exoapplication.helper.livedata

import android.os.Looper
import androidx.lifecycle.MutableLiveData
import com.waynils.exoapplication.helper.extension.removeAny

class MutableLiveList<T>(_initValues: Collection<T> = mutableListOf()) : MutableLiveData<List<T>>(), MutableList<T> {
    ...
    fun replace(list: List<T>) {
        doAndUpdate {
            this@MutableLiveList.list.apply {
                clear()
                addAll(list)
            }
            true
...
fun <T> mutableLiveListOf(vararg elements: T) = MutableLiveList(elements.toMutableList())